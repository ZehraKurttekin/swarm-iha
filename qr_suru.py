...
class QRSuruOperasyonu(Node):
    def __init__(self):
        ...
        self.lider_arama_x = 0.0;
        self.lider_arama_yonu = 1
        self.yanci_arama_x = 0.0;
        self.yanci_arama_yonu = -1

        self.lider_pose_sub = self.create_subscription(VehicleLocalPosition, '/px4_1/fmu/out/vehicle_local_position_v1',
                                                       self.l_pose_cb, qos_profile)
        self.lider_status_sub = self.create_subscription(VehicleStatus, '/px4_1/fmu/out/vehicle_status_v1',
                                                         self.l_status_cb, qos_profile)
        self.kanat_pose_sub = self.create_subscription(VehicleLocalPosition, '/px4_2/fmu/out/vehicle_local_position_v1',
                                                       self.k_pose_cb, qos_profile)
        self.kanat_status_sub = self.create_subscription(VehicleStatus, '/px4_2/fmu/out/vehicle_status_v1',
                                                         self.k_status_cb, qos_profile)
        ...
        self.lider_cam_sub = self.create_subscription(Image, '/px4_1/camera/image_raw', self.lider_kamera_cb,
                                                      qos_profile_sensor_data)
        self.kanat_cam_sub = self.create_subscription(Image, '/px4_2/camera/image_raw', self.kanat_kamera_cb,
                                                      qos_profile_sensor_data)
    ...
    def l_status_cb(self, msg):
        self.lider_durum = msg.nav_state; self.lider_arm = msg.arming_state

    def k_status_cb(self, msg):
        self.kanat_durum = msg.nav_state; self.kanat_arm = msg.arming_state

    def l_pose_cb(self, msg):
        self.lider_x = msg.x;
        self.lider_y = msg.y;
        self.lider_z = -msg.z;
        self.lider_yaw = msg.heading

    def k_pose_cb(self, msg):
        self.kanat_x = msg.x;
        self.kanat_y = msg.y;
        self.kanat_z = -msg.z;
        self.kanat_yaw = msg.heading
    ...
    def komut_gonder(self, komut, drone_id, p1=0.0, p2=0.0):
        ...
        if drone_id == 1:
            self.lider_cmd_pub.publish(msg)
        else:
            self.kanat_cmd_pub.publish(msg)
    ...
    def lider_kamera_cb(self, msg):
        ...
        if bbox is not None and len(bbox) > 0:
            ...
            qr_x = x + bw // 2;
            qr_y = y + bh // 2
            self.l_err_x = float(qr_x - self.merkez_x);
            self.l_err_y = float(qr_y - self.merkez_y)

            cv2.putText(cv_image, f"ALAN: {int(self.l_alan)}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255),
                        2)
        ...
        cv2.putText(cv_image, f"MOD: {self.operasyon_durumu}", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255),
                    2)
        cv2.imshow("LIDER EKRANI", cv_image);
        cv2.waitKey(1)
    ...
    def kanat_kamera_cb(self, msg):
        ...
        if bbox is not None and len(bbox) > 0:
            ...
            qr_x = x + bw // 2;
            qr_y = y + bh // 2
            self.y_err_x = float(qr_x - self.merkez_x);
            self.y_err_y = float(qr_y - self.merkez_y)

            self.dinamik_tolerans = max(20, int(np.sqrt(self.y_alan) * 0.2))

            cv2.putText(cv_image, f"ALAN: {int(self.y_alan)}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255),
                        2)
            cv2.putText(cv_image, f"TOLERANS: {self.dinamik_tolerans}", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                        (255, 255, 255), 2)

            renk = (0, 255, 0) if abs(self.y_err_x) < self.dinamik_tolerans and abs(
                self.y_err_y) < self.dinamik_tolerans else (0, 0, 255)
        ...
        cv2.putText(cv_image, f"MOD: {self.operasyon_durumu}", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    (255, 255, 255), 2)
        cv2.imshow("YANCI EKRANI", cv_image);
        cv2.waitKey(1)
    ...
    def sistem_dongusu(self):
        if self.operasyon_durumu != "GOREV_TAMAM":
            ...
            self.lider_mode_pub.publish(offboard_msg);
            self.kanat_mode_pub.publish(offboard_msg)

        lider_sp = TrajectorySetpoint();
        kanat_sp = TrajectorySetpoint()

        if self.operasyon_durumu == "KALKIS":
            lider_sp = self.setpoint_konum(0.0, 0.0, 5.0);
            kanat_sp = self.setpoint_konum(0.0, 3.0, 5.0)
            ...
            if self.lider_arama_x > 10.0:
                self.lider_arama_yonu = -1
            elif self.lider_arama_x < -2.0:
                self.lider_arama_yonu = 1

            self.yanci_arama_x += 0.02 * self.yanci_arama_yonu
            if self.yanci_arama_x < -10.0:
                self.yanci_arama_yonu = 1
            elif self.yanci_arama_x > 2.0:
                self.yanci_arama_yonu = -1
            ...
            if self.l_gordu:
                self.l_kayip = 0
                lider_sp, _, _ = self.hizala_ve_donustur(self.l_err_x, self.l_err_y, self.l_prev_x, self.l_prev_y,
                                                         self.lider_yaw, vz_down=0.0)
                self.l_prev_x = self.l_err_x;
                self.l_prev_y = self.l_err_y

                if abs(self.l_err_x) < 20 and abs(self.l_err_y) < 20:
                    self.kilitli_hedef_x = self.lider_x;
                    self.kilitli_hedef_y = self.lider_y
                    self.operasyon_durumu = "ORTAK_HEDEF_YAKLASMA"
            else:
                self.l_kayip += 1
                if self.l_kayip < 20:
                    lider_sp, _, _ = self.hizala_ve_donustur(0.0, 0.0, 0.0, 0.0, self.lider_yaw, vz_down=0.0)
                else:
            ...
            if self.y_gordu:
                self.y_kayip = 0
                kanat_sp, _, _ = self.hizala_ve_donustur(self.y_err_x, self.y_err_y, self.y_prev_x, self.y_prev_y,
                                                         self.kanat_yaw, vz_down=0.0)
                self.y_prev_x = self.y_err_x;
                self.y_prev_y = self.y_err_y

                if abs(self.y_err_x) < 20 and abs(self.y_err_y) < 20:
                    self.kilitli_hedef_x = self.kanat_x;
                    self.kilitli_hedef_y = self.kanat_y
                    self.operasyon_durumu = "ORTAK_HEDEF_YAKLASMA"
            else:
                self.y_kayip += 1
                if self.y_kayip < 20:
            ...
            if self.y_gordu:
                ...
                if self.y_alan > 120000 or self.kanat_z <= 0.8:
                    ...
                    if abs(self.y_err_x) > self.dinamik_tolerans or abs(self.y_err_y) > self.dinamik_tolerans:
                        kanat_sp, vx, vy = self.hizala_ve_donustur(self.y_err_x, self.y_err_y, self.y_prev_x,
                                                                   self.y_prev_y, self.kanat_yaw, vz_down=0.0)

                    else:
                        kanat_sp, vx, vy = self.hizala_ve_donustur(self.y_err_x, self.y_err_y, self.y_prev_x,
                                                                   self.y_prev_y, self.kanat_yaw, vz_down=0.4)
        ...
        if self.operasyon_durumu != "GOREV_TAMAM":
            self.lider_traj_pub.publish(lider_sp);
            self.kanat_traj_pub.publish(kanat_sp)
